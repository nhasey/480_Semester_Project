...
ai_dataset = torchvision.datasets.ImageFolder(root=pathAI, transform=transform)
# Set all labels in AI dataset to 0
ai_dataset.targets = [0] * len(ai_dataset)


human_dataset = torchvision.datasets.ImageFolder(root=pathHuman, transform=transform)
# Set all labels in Human dataset to 1
human_dataset.targets = [1] * len(human_dataset)

# Combine the datasets
combined_dataset = ConcatDataset([ai_dataset, human_dataset])
...
class VerifierCNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32 * 16 * 16, 2)  # Adding extra conv layer
